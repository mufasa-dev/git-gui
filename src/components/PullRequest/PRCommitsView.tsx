            ...
            <For each={commits()}>
              {(commit) => (
                <div class="group flex items-start gap-4 p-4 hover:bg-gray-700/30 transition-colors cursor-pointer" 
                      onClick={() => props.selectCommit(commit.oid)}>
                  {/* Avatar do Autor */}
                  <img 
                    src={commit.author.avatarUrl} 
                    alt={commit.author.user?.login || commit.author.name} 
                    class="w-8 h-8 rounded-full border border-gray-300 dark:border-gray-700 mt-1"
                  />

                  <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between gap-2 mb-1">
                      <span class="text-xs font-bold text-gray-800 dark:text-gray-300 truncate">
                        <CommitMessage message={commit.message.split('\n')[0]} />
                      </span>
