...
export default function CloneRepositoryModal(props: { 
  ...
  const getRepoName = (u: string) => {
    ...
    return last ? last.replace('.git', '').trim() : '';
  };

  createEffect(on(url, (currentUrl) => {
    const name = getRepoName(currentUrl);
    const currentPath = path();

    if (name && currentPath) {
      if (currentPath.endsWith(name)) return;

      const cleanPath = currentPath.replace(/\/+$/, ''); 
      setPath(`${cleanPath}/${name}`);
    }
  }, { defer: true }));
  ...
  return (
    <Show when={props.isOpen}>
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-gray-800 w-full max-w-md rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden transform transition-all">
          ...
          <form onSubmit={handleSubmit} class="p-6 space-y-4">
            <div>
              ...
              <input 
                ...
                class="w-full p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                value={url()}
            ...
            <div>
              ...
              <div class="flex gap-2">
                <input 
                  ...
                  class="flex-1 p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                  value={path()}
                ...
                class="px-4 bg-gray-200 dark:bg-gray-700 rounded-xl hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"
                >
