...
const hunkHeaderPattern = { 
    name: "hunk-header", 
    regex: /^(@@\s.*?\s@@)\s*(.*)/g, 
    formatter: (_m: string, coords: string, funcContext: string) =>
...
const mdPatterns = [
    ...
    { 
        name: "md-link", 
        regex: /(\[.*?\])(\(.*?\))/g, 
        formatter: (_m: string, p1: string, p2: string) =>
...
function getPattern(fileName: string, dynamicKeywordRegex: RegExp) {
    ...
    let pattern = [
        ...
        { 
            name: "comment", 
            regex: /(?<!:)(\/\/.*)|\/\*[\s\S]*?\*\//g, 
            class: "text-green-600 dark:text-green-500 italic" 
        },
        // Strings
        { 
            name: "string-normal", 
        ...
        { 
            ...
            formatter: (match: string) => {
                if (!match.includes('${')) {
                    return `<span class="text-amber-600 dark:text-orange-400">${match}</span>`;
                }

                const highlightedTemplate = match.replace(/(\$\{)(.*?)(\})/g, (_m, p1, p2, p3) => {
                    return `<span class="text-blue-500 dark:text-sky-400 font-bold">${p1}</span><span class="text-gray-900 dark:text-gray-100">${p2}</span><span class="text-blue-500 dark:text-sky-400 font-bold">${p3}</span>`;
                });

                return `<span class="text-amber-600 dark:text-orange-400">${highlightedTemplate}</span>`;
            }
        },
        ...
        { 
            ...
            formatter: (_m: string, p1: string, p2: string) => `<span class="text-gray-400">${p1}</span> <span class="text-teal-500 dark:text-teal-300">${p2}</span>`
        },
        // Componentes (Maiúscula)
        { 
            name: "component", 
            regex: /(&lt;\/?)([A-Z][a-zA-Z0-9_]*)/g, 
            formatter: (_m: string, p1: string, p2: string) => `<span class="text-gray-500">${p1}</span><span class="text-teal-500 dark:text-teal-300">${p2}</span>`
        },
        // Tags Nativas (Minúscula)
        { 
            name: "tag", 
            regex: /(&lt;\/?)([a-z][a-z0-9-]*)/g, 
            formatter: (_m: string, p1: string, p2: string) => `<span class="text-gray-500">${p1}</span><span class="text-blue-500 dark:text-sky-400">${p2}</span>`
        },
        // Atributos
