...
const baseToast = (title: string, message: string, colorClass: string, isVisible: boolean, toastId: string) => (
  <div class={`${isVisible ? 'animate-bounce-in' : 'animate-fade-out'} 
    max-w-md w-80 bg-white dark:bg-slate-800 shadow-2xl rounded-lg border border-gray-200 dark:border-slate-700 flex overflow-hidden ring-1 ring-white/5`}>
    ...
    <div class="flex-1 p-3">
      ...
      <p class="text-sm text-gray-800 dark:text-slate-100 font-medium leading-tight">{message}</p>
    </div>
    <button 
      onClick={() => toast.dismiss(toastId)}
      class="px-3 text-slate-500 dark:hover:text-white hover:bg-slate-200/50 dark:hover:bg-slate-700/50 transition-all text-lg"
    >
      ✕
    </button>
  </div>
